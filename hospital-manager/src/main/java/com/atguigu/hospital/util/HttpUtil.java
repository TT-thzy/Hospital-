...
public final class HttpUtil {
	...
	public static byte[] send(String strUrl, String reqmethod, byte[] reqData) {
		try {
			...
			httpcon.setDoOutput(true); //设置是否输出
			httpcon.setDoInput(true); //设置是否读取
			httpcon.setUseCaches(false);// 设置是否使用缓存
			httpcon.setInstanceFollowRedirects(true);  // 设置此 HttpURLConnection 实例是否应该自动执行 HTTP 重定向
			httpcon.setConnectTimeout(CONN_TIMEOUT); // 连接超时时间
			httpcon.setReadTimeout(READ_TIMEOUT);
			httpcon.setRequestMethod(reqmethod); // 请求方式
			httpcon.connect();	// 连接
			if (reqmethod.equalsIgnoreCase(POST)) { //写入参数到请求中
